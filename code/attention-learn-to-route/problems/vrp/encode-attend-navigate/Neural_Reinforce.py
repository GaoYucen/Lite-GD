...
class Actor(object):
    ...
    def encode_decode(self):
        ...
        W_ref = tf.compat.v1.get_variable("W_ref",[1, n_hidden, self.num_units],initializer=self.initializer)
        W_q = tf.compat.v1.get_variable("W_q",[self.query_dim, self.num_units],initializer=self.initializer)
        v = tf.compat.v1.get_variable("v",[self.num_units],initializer=self.initializer)
        ...
        W_1 =tf.compat.v1.get_variable("W_1",[n_hidden, self.query_dim],initializer=self.initializer) # update trajectory (state)
        W_2 =tf.compat.v1.get_variable("W_2",[n_hidden, self.query_dim],initializer=self.initializer)
        W_3 =tf.compat.v1.get_variable("W_3",[n_hidden, self.query_dim],initializer=self.initializer)
    ...
    def build_critic(self):
        ...
        with tf.variable_scope("ffn"): #  2 dense layers for predictions
            h0 = tf.compat.v1.layers.dense(frame, self.num_neurons_critic, activation=tf.nn.relu, kernel_initializer=self.initializer)
            w1 = tf.compat.v1.get_variable("w1", [self.num_neurons_critic, 1], initializer=self.initializer)
