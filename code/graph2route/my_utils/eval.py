...
def route_acc(pred, label, top_n): # 不一样是0，一样是1
    """
    calculate ACC@k
    """
...
class Metric(object):
    ...
    def to_dict(self) -> Dict:
        result = {f'hr@{i + 1}': self.hr[i].avg for i in range(self.max_seq_len)}
        result.update({f'acc@{i + 1}': self.acc[i].avg for i in range(self.max_seq_len)})
